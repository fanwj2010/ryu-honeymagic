...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def _dump_alert(self, ev):
        ...
        if "DROP" not in alert_rt:
           #Forward the ACK_PSH pkt 
           if self.front_end in alert_rt:
              self.logger.info("Forward pkt to frontend...\n")
              ...
              data = sess.payload_pkt_data
              out = parser.OFPPacketOut(datapath=datapath,buffer_id=ofproto.OFP_NO_BUFFER,
                                      in_port=ofproto.OFPP_CONTROLLER, actions=actions,data=data) 
              datapath.send_msg(out)
              payload_tcp =  sess.payload_pkt.get_protocol(tcp.tcp)
              self.logger.info("ACK_PSH sent [seq = %s , ack = %s]", payload_tcp.seq, payload_tcp.ack)
...
###     ## Attacker launches Session: Attacker --> Honeypot
        ...
        if pkt_tcp.bits == 0x2 and sess.getState() == Session.SESSION_INITIAL:
           ...
           hp_mac = str(self.config[pkt_ipv4.dst]['MAC'])
           if dpid == dpid_ovs: 
              out_port_number = int(self.config[pkt_ipv4.dst][self.front_end])
	      self.mac_to_port[dpid][hp_mac] = out_port_number
	      out_port = out_port_number
	      sess.setOutport(out_port)
              self.logger.info("After lookup [dst mac = %s, self.mac_to_port[dpid=%s] = %s]", pkt_ethernet.dst, dpid, self.mac_to_port[dpid])
        ...
        #ACK
             ...
             out = parser.OFPPacketOut(datapath=datapath,buffer_id=ofproto.OFP_NO_BUFFER,
                                      in_port=in_port, actions=actions,data=data)                      
             datapath.send_msg(out)
             self.logger.info("ACK sent [seq = %s , ack = %s], TCP connection established.", pkt_tcp.seq, pkt_tcp.ack)  
             ...
             try:
	           ...
	           out = parser.OFPPacketOut(datapath=datapath, buffer_id=ofproto.OFP_NO_BUFFER,
                   ...
                   self.logger.info("Attacker launched Session [%s:%s] in dictionary is deleted.\n", pkt_ipv4.src, pkt_tcp.src_port)
                    ...
                    #Forward the ACK_PSH pkt 
                    if self.front_end in alert_rt:
                       self.logger.info("Forward the pkt to frontend.") 
                       #if pkt_ethernet.dst in self.mac_to_port[dpid]:
                       #   out_port = self.mac_to_port[dpid][pkt_ethernet.dst]                     
                       #else:
                       #   out_port = ofproto.OFPP_FLOOD
                       ...
                       payload_tcp =  sess.payload_pkt.get_protocol(tcp.tcp)
                       self.logger.info("ACK_PSH sent [seq = %s , ack = %s]", payload_tcp.seq, payload_tcp.ack)
                       sess.setState(Session.A_TCP_FORWARD)
...
###     ## Attacker launched Session: Attacker <-- Honeypot
        #SYN_ACK
        if pkt_tcp.bits == 1 << 1 | 1 << 4 and sess.getState() == Session.A_TCP_HS_PLAY:
           self.logger.info("Attacker launched Session state = %s", sess.getState())
           self.logger.info("SYN_ACK recv [seq = %s , ack = %s]", pkt_tcp.seq, pkt_tcp.ack)
           sess.saveFRONTsynseq(pkt_tcp.seq)
           dst = pkt_ethernet.dst
           src = pkt_ethernet.src
        ...
        #ACK
        elif pkt_tcp.bits & 1 << 4 and sess.getState() == Session.A_TCP_FORWARD:
           self.logger.info("Attacker launched Session state = %s", sess.getState())
           self.logger.info("ACK recv [seq = %s , ack = %s]", pkt_tcp.seq, pkt_tcp.ack)
           ...
           self.logger.info("ACK resp [seq = %s , ack = %s]", pkt_tcp.seq, pkt_tcp.ack)
           self.logger.info("Attacker launched Session state = %s", sess.getState())
           return 
...
###     ## Attacker launched Session Replay: Controller <-- Honeypot
        #SYN_ACK               
        elif pkt_tcp.bits == 1 << 1 | 1 << 4 and sess.getState() == Session.A_TCP_HS_REPLAY:
           self.logger.info("Attacker launched Replay Session state = %s", sess.getState())
           self.logger.info("SYN_ACK recv [seq = %s , ack = %s]", pkt_tcp.seq, pkt_tcp.ack)
           ...
           if dpid !=  dpid_ovs:              
        	 ...
        	 out_port = ofproto.OFPP_FLOOD
              ...
              ack_diff = sess.getACKDiff(sess.honeypot_syn_seq, sess.frontend_syn_seq)
              ...
              seq_diff = sess.getSEQDiff(sess.frontend_syn_seq, sess.honeypot_syn_seq)
